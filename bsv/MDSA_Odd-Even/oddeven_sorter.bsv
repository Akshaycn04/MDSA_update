...
module mk_sorting_network(Ifc_sorting_network);
   ...
   // Stage registers for 6-stage pipelined sorting
   ...
   rule rl_start_sort (rg_stage == INIT && rg_data_valid && !rg_sort_complete);
      ...
      rg_stage <= STAGE_1;
      rg_data_valid <= False;
      //$display("Starting sort process with data: %h", rg_input_data);
   endrule
   ...
   rule rl_stage_6 (rg_stage == STAGE_6);
      ...
      // REVERSE for descending order output
      OEH final_sorted = reverse(temp_sorted);
   ...
   method Action ma_put_data (OEH oeh_in) if (rg_stage == INIT && !rg_data_valid);
      ...
      //$display("Input data received: %h", oeh_in);
   endmethod
