...
interface Ifc_sorting_network;
   method Action ma_put_data (OEH oeh_in);
   method OEH mv_get_sorted_data();
   method Bool mv_is_ready();
endinterface

(* synthesize *)
module mk_sorting_network(Ifc_sorting_network);
   
   // Input and output registers
   Reg#(OEH) rg_input_data <- mkReg(replicate(0));
   Reg#(OEH) rg_output_data <- mkReg(replicate(0));
   ...
   Reg#(OEH) rg_stage_0 <- mkReg(replicate(0));
   Reg#(OEH) rg_stage_1 <- mkReg(replicate(0));
   Reg#(OEH) rg_stage_2 <- mkReg(replicate(0));
   Reg#(OEH) rg_stage_3 <- mkReg(replicate(0));
   Reg#(OEH) rg_stage_4 <- mkReg(replicate(0));
   Reg#(OEH) rg_stage_5 <- mkReg(replicate(0));
   Reg#(OEH) rg_stage_6 <- mkReg(replicate(0));
   Reg#(OEH) rg_stage_7 <- mkReg(replicate(0));
   ...
   // Rule: Start sorting process
   rule rl_start_sort (rg_stage == INIT && rg_data_valid && !rg_sort_complete);
      rg_stage_0 <= rg_input_data;
      rg_stage <= STAGE_1;
      rg_data_valid <= False;
      fn_display($format("Starting sort process with data: %h", rg_input_data));
   endrule
   
   // Rule: Stage 1 - (0,1), (2,3), (4,5), (6,7)
   rule rl_stage_1 (rg_stage == STAGE_1);
      ...
      rg_stage <= STAGE_2;
      fn_display($format("Stage 1 complete: %h", temp_stage_1));
   endrule
   
   // Rule: Stage 2 - (0,2), (1,3), (4,6), (5,7)
   rule rl_stage_2 (rg_stage == STAGE_2);
      ...
      rg_stage <= STAGE_3;
      fn_display($format("Stage 2 complete: %h", temp_stage_2));
   endrule
   
   // Rule: Stage 3 - (1,2), (0,4), (5,6), (3,7)
   rule rl_stage_3 (rg_stage == STAGE_3);
      ...
      rg_stage <= STAGE_4;
      fn_display($format("Stage 3 complete: %h", temp_stage_3));
   endrule
   
   // Rule: Stage 4 - (1,4), (2,5), (3,6)
   rule rl_stage_4 (rg_stage == STAGE_4);
      ...
      rg_stage <= STAGE_5;
      fn_display($format("Stage 4 complete: %h", temp_stage_4));
   endrule
   
   // Rule: Stage 5 - (2,4), (3,5)
   rule rl_stage_5 (rg_stage == STAGE_5);
      ...
      rg_stage <= STAGE_6;
      fn_display($format("Stage 5 complete: %h", temp_stage_5));
   endrule
   
   // Rule: Stage 6 - (1,2), (3,4), (5,6)
   rule rl_stage_6 (rg_stage == STAGE_6);
      ...
      rg_stage <= STAGE_7;
      fn_display($format("Stage 6 complete: %h", temp_stage_6));
   endrule
   
   // Rule: Stage 7 - (2,3), (4,5)
   rule rl_stage_7 (rg_stage == STAGE_7);
      ...
      rg_stage <= STAGE_8;
      fn_display($format("Stage 7 complete: %h", temp_stage_7));
   endrule
   ...
   rule rl_stage_8 (rg_stage == STAGE_8);
      ...
      OEH final_sorted = rg_stage_7;
      final_sorted[3] = sorted_34[0]; final_sorted[4] = sorted_34[1];
      
      rg_output_data <= final_sorted;
      rg_sort_complete <= True;
      rg_stage <= INIT;
      fn_display($format("Sorting complete: %h", final_sorted));
   endrule
   
   // Methods
   method Action ma_put_data (OEH oeh_in) if (rg_stage == INIT && !rg_data_valid);
      rg_input_data <= oeh_in;
      rg_data_valid <= True;
      if (rg_sort_complete) begin
         rg_sort_complete <= False;
      end
      fn_display($format("Input data received: %h", oeh_in));
   endmethod
   
   method OEH mv_get_sorted_data() if (rg_sort_complete);
